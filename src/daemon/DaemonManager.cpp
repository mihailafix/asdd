...
{
    // prepare command line arguments and pass to kosond
    ...
    qDebug() << "starting kosond " + m_kosond;
    ...
    // Start kosond
    bool started = m_daemon->startDetached(m_kosond, arguments);
    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM kosond.exe");
#else
                QProcess::execute("pkill kosond");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./kosond status` returns BUSY when syncing.
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to kosond
#ifdef Q_OS_WIN
    m_kosond = QApplication::applicationDirPath() + "/kosond.exe";
#elif defined(Q_OS_UNIX)
    m_kosond = QApplication::applicationDirPath() + "/kosond";
#endif

    if (m_kosond.length() == 0) {
